...
def clean_text(text):
    # Remove 9-digit numbers
    text = re.sub(r'\b\d{9}\b', '', text)
    # Remove 5-digit numbers
...
def extract(text):
    ...
    text = text.replace(r"PO Box Refund", "")
...
def cleanCompanies(series):
    clean_series = series.str.lower()  # Convert to lowercase
    clean_series = clean_series.str.replace(r"\bPO Box Refund\b", "", regex=True)  # Replace whole word "PO Box Refund"
    clean_series = clean_series.str.replace(r"\d", "", regex=True)  # Remove digits
    clean_series = clean_series.str.replace(r"[^\w\s]", "", regex=True)  # Remove non-alphanumeric characters except spaces
    clean_series = clean_series.str.strip()  # Strip leading and trailing whitespace
    return clean_series
